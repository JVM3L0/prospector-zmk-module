...
static lv_obj_t *create_mod_label(lv_obj_t *parent, const char *symbol) {
    ...
    /* Reduz a escala visual do ícone (256 = 100%, 192 = 75%, 160 = ~62%) */
    lv_obj_set_style_transform_zoom(label, 192, LV_PART_MAIN);
...
int zmk_widget_modifier_indicator_init(struct zmk_widget_modifier_indicator *widget, lv_obj_t *parent) {
    ...
    for (int i = 0; i < 4; i++) {
        widget->mod_labels[i] = create_mod_label(widget->obj, modifier_order_get_symbol(i));
    }
